...
export const AuctionCreateView = () => {
      ...
      if (attributes.items.length > 0) {
        ...
        if (
          ...
          item.winningConfigType =
            ...
            (walletContext?.publicKey || SystemProgram.programId).toBase58()
  ...
  return (
    <>
      {!walletContext?.connected && <ConnectButton loading={isLoading} style={{ margin: '0.5rem auto', display: 'block' }} type="primary" size="large" shape="round">
        {isLoading ? '' : 'Connect to trade apes'}
      </ConnectButton>}
      {walletContext?.connected && <Row className="auction-create" style={{ paddingTop: width < 768 ? 0 : 50 }}>
        {stepsVisible && (
          <Col span={24} md={4}>
            <Steps
              progressDot
